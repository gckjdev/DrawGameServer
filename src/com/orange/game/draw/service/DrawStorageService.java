...
public class DrawStorageService {
    ...
    public void storeDraw(final int sessionId, final GameUser user, final String wordText, final int wordLevel,
    	...
    	executor.execute(new Runnable(){
			...
			public void run() {
				...
				if (data.length < MIN_DRAW_DATA_LEN){
					ServerLog.info(sessionId, "save data into DB, but data too short, data bytes="+data.length);
					return;
				}
				...
				ServerLog.info(sessionId, "save data into DB, data bytes="+data.length);
			}
	...
	public BasicDBObject randomGetDraw(int sessionId, int language, Set<String> excludeUserIdList){
    	...
    	ServerLog.info(sessionId, "<randomGetDraw> query = "+query.toString());
		...
		if (obj != null){
			...
			ServerLog.info(sessionId, "<randomGetDraw> update = "+keyQuery.toString() + ", update="+update.toString());
			dbClient.findAndModify(DBConstants.T_DRAW, keyQuery, update);
		}

		if (obj == null){
    		ServerLog.info(sessionId, "random fetch data from DB, but no record return");
    		return null;
    	}
    	
    	byte[] data = (byte[])obj.get(DBConstants.F_DRAW_DATA);
    	if (data == null){
    		ServerLog.info(sessionId, "random fetch data from DB, but draw data null?");
    		return null;    		
    	}
    	
    	ServerLog.info(sessionId, "random fetch data from DB, data bytes="+data.length);
	...
	public void storeGuessWord(final int sessionId, final String word, final int language,
		...
		executor.execute(new Runnable(){
			...
			public void run() {
		    	...
		    	ServerLog.info(sessionId, "<storeGuessWord> query="+query.toString()+", update="+update.toString());
				ServerLog.info(sessionId, "save guess word data into DB, word count ="+collection.size());
