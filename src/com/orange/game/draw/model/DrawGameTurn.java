...
public class DrawGameTurn {
	...
	public DrawGameTurn(int sessionId, int round, String word, int level, int language, GameUser currentPlayUser) {
	...
	public void userGuessWord(GameUser user, String guessWord){
		...
		int guessDifficultLevel = 1; // TODO user.guessDifficultLevel;
		
		if (userId == null || guessWord == null)
			return;
		
		if (!DrawRobotManager.isRobotUser(userId) &&
		...
		if (isCorrect && !guess.isCorrect){
			...
			*/
			...
			ServerLog.info(sessionId, "<userGuessWord> correct, gain coins = " + finalCoins + ", diff level="+guess.guessDifficultLevel);
		}
...
//		return (correctCount >= guessUserCount);
	...
	public void calculateDrawUserCoins(GameUser drawUser) {
		...
		*/		
		...
		ServerLog.info(sessionId, "drawUserCoins set to "+drawUserCoins);
	}
...
//			return uw.finalCoins;
		...
		if (value == null){
			...
			ServerLog.debug(sessionId, "get user coins = "+value.intValue());
	...
	public void storeDrawData() {
		...
		if (DrawRobotManager.isRobotUser(drawUserId)){
			ServerLog.info(sessionId, "skip store draw data due to robot draw user");
			return;
		}
		...
		PBDraw draw = PBDraw.newBuilder().setUserId(drawUserId)
			...
			.setNickName(drawUser.getNickName())
			.setAvatar(drawUser.getAvatar())
			.build();
